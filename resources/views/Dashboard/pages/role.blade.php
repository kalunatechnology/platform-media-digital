                            ...
                            <main class="d-flex flex-column gap-3 grow">
                                <section class="d-flex gap-2 items-center justify-content-between">
                                    <div class="d-flex align-items-center ml-auto">
                                        <a href="{{ url('/backoffice/addrole') }}" type="button" class="btn btn-primary rounded-lg">
                                        ...
                                        <div class="card-body px-0 pt-0 pb-2">
                                            <div class="table-responsive">
                                                <table class="table align-items-center mb-0 table-hover">
                                                    ...
                                                    @endphp
                                                    <tbody>
                                                        @foreach ($card['get_data'] as $role)
                                                        <tr>
                                                            <td class="text-center">{{ $nomor++ }}</td>
                                                            <td class="text-center">{{ $role->name}}</td>
                                                            <td>
                                                                <div class="d-flex">
                                                                    <a href="{{ url('/backoffice/' . $role->id . '/editrole') }}"
                                                                        class="btn btn-primary btn-sm bg-primary mr-1 text-white">Edit</a>
                                                                    <button class="btn btn-danger btn-sm bg-danger mx-1 text-white" onclick="confirmDelete({{ $role->id }})">
                                                                        Delete
                                                                    </button>
                                                                </div>
                                                            </td>
