...
<div class="ml-sidebar">
    ...
    <div class="content">
        ...
        <div class="card total-timses-card">
            <div class="card-body">
                <h3>Data Archived Artikel {{ $card['username'] }}</h3>
        ...
        <!-- Search Bar -->
        <form method="GET" action="{{ url('/backoffice/archived') }}" class="form-search">
            <label for="search">
        ...
        <div class="keterangan">
            <p>1. Anda bisa mempublish ulang selama masa penanyangan masih berlaku.  <br>
                2. Atau anda bisa menghapus artikel jika tidak di inginkan lagi.</p>
        </div>
        <br><br>
                        ...
                        <tbody>
                            ...
                            @foreach ($card['data_draft'] as $artikel)
                                <tr>
                                    ...
                                    <td class="d-flex">
                                        <button class="btn btn-warning btn-sm bg-warning mx-1 text-black" onclick="confirmUpdate({{ $artikel->id }})">
                                            Terbitkan Lagi
                                        </button>
    ...
    function confirmDelete(artikelId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`/backoffice/artikel_archived/${artikelId}`, {
                    method: "DELETE",
                    headers: {
...
<script>
    function confirmUpdate(articleId) {
        Swal.fire({
            title: "Anda yakin?",
            text: "Artikel ini akan diterbitkan ulang dan diganti status menjadi Published!",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#d33",
            cancelButtonColor: "#3085d6",
            ...
            if (result.isConfirmed) {
                fetch(`/backoffice/${articleId}/publish_ulang`, {
                    method: "GET",
                    headers: {
                        "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content,
                        "Content-Type": "application/json"
                ...
                .then(data => {
                    if (data.status === "success") {
                        Swal.fire({
                            ...
                            location.reload(); // Reload untuk melihat perubahan status
                        });
                    } else {
